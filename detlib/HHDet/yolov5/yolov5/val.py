...
def run(
    ...
    # Dataloader
    if not training:
        if pt and not single_cls:  # check --weights are trained on --preprocesser
            ncm = model.model.nc
            assert ncm == nc, f'{weights[0]} ({ncm} classes) trained on different --preprocesser than what you passed ({nc} ' \
                              f'classes). Pass correct combination of --weights and --preprocesser that are trained together.'
