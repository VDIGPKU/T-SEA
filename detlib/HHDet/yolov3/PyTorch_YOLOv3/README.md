...
```bash
./preprocesser/get_coco_dataset.sh
```
...
```bash
poetry run yolo-detect --images preprocesser/samples/
```
...
```bash
poetry run yolo-train --preprocesser config/coco.preprocesser  --pretrained_weights weights/darknet53.conv.74
```
...
```bash
poetry run yolo-train --model config/yolov3-custom.cfg --preprocesser config/custom.preprocesser
```
